...
abstract class _DownloadController with Store {
  ...
  String _danmakuFilePath(String downloadDirectory, {int? episode}) {
    return episode == null
        ? '$downloadDirectory/danmaku.json'
  ...
  Future<({List<DanmakuEntry> danmakus, int danDanBangumiID})?>
      _readDanmakuFromFile(String downloadDirectory, {int? episode}) async {
    if (downloadDirectory.isEmpty) return null;
    final file = File(_danmakuFilePath(downloadDirectory, episode: episode));
  ...
  Future<({List<DanmakuEntry> danmakus, int danDanBangumiID})?>
      readDirectoryDanmaku(String directory, {int? episode}) {
    return _readDanmakuFromFile(directory, episode: episode);
  }
