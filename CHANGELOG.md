...
- BUG 修复：弹幕轴自动检测在小样本下被离群弹幕误导，产生高置信度错误推荐
  - 背景：`_percentile` 分位取整使弹幕少于约 200 条时 P0.5/P99.5 就是 min/max（离群剔除失效），且 `tailBeyond`/`headBeyond` 越界信号无任何守卫；实测 80 条正常弹幕 + 1 条越界 120s 的离群弹幕即触发「提前 2 分钟、可信度中」的误推荐，应用后错误偏移写入分集作用域持续整集
  - 修复：新增越界群采信校验 `_isBeyondClusterSupported`——越界弹幕需达到条数下限（至少 1 条，大样本按 0.5% 比例提门槛）且与弹幕主体之间无空隙（超过 2 倍对齐容差视为孤立离群不采信）；轴头 P0.5 分位索引改为向上取整，保证小样本至少剔除首条离群弹幕；`_isHardEdge` 方向参数化，轴头方向相邻窗口为空时不再误判硬边界（原实现轴头 far 窗口恒空、恒走兜底分支）
  - 修复：检测弹窗展示期间换集后点「应用推荐偏移」会把旧集检测结果写入新集作用域——现在捕获检测时的弹幕绑定，应用前校验不一致则取消并提示
  - 修复：视频时长等待超时（慢源 5s 内未就绪）后本集永久放弃检测——改为间隔 3s 后再补一轮等待（会话守卫保证换集安全）
  - 其他：episodeId 未知（=0）时应用推荐偏移记录番剧级作用域日志，便于排查跨分集遮蔽
  - 验证：正式检测器代码经纯 Dart 抽取运行 23 项断言全部通过（17 项现有基线含精确数值 + 6 项新回归）；`flutter test` 请在本地终端复核（代理会话内 dart/flutter 工具受安全层管道限制无法启动）
...
- 同步上游 2.3.2 ~ 2.3.4（批次 5：基线 23610a52 → 88a8ec59 共 28 个提交，按「逐提交内容移植 + 回植 fork 功能」完成 18 项，跳过 10 项；Flutter 保持 3.47.2，逐项登记见 UPSTREAM_SYNC.md 批次 5）
  - 批次 5A 低冲突 UI 修复：搜索/收藏卡片评分可见性（4b29571e）、搜索布局变化保持输入连接（5006efac）、tab 路由不匹配修复（3bfd5d1f）、统一操作按钮高度（2624e0c0）、我的页竖屏布局简化（cd7c0f88）
    - 相关文件: lib/pages/collect/collect_library_card.dart, lib/pages/collect/collect_library_view.dart, lib/pages/collect/collect_page.dart, lib/pages/search/search_page.dart, lib/pages/search/search_widgets.dart, lib/pages/settings/interface_settings.dart, lib/pages/index_module.dart, lib/pages/index_page.dart, lib/pages/menu/menu.dart, lib/bean/widget/state_presentation.dart, lib/pages/my/my_space_view.dart, lib/pages/timeline/timeline_options.dart
  - 批次 5B WebDAV：启动初始化尊重历史同步开关（1317f20d）；新增弹幕屏蔽规则 WebDAV 同步（dc5bd860，含规则仓库/同步服务/各设置页接入），恢复上游更新的 webdav 服务测试
    - 相关文件: lib/pages/init_page.dart, lib/modules/danmaku/danmaku_shield_rule.dart, lib/modules/danmaku/danmaku_shield_sync.dart, lib/repositories/danmaku_shield_repository.dart, lib/services/sync/danmaku_shield_sync_service.dart, lib/services/sync/webdav.dart, lib/pages/my/my_controller.dart, lib/pages/settings/danmaku/danmaku_shield_editor.dart, lib/pages/settings/settings_module.dart, lib/pages/settings/sync/sync_settings_page.dart, lib/pages/settings/sync/webdav_sync_page.dart, lib/services/storage/settings_keys.dart, lib/core_module.dart, test/webdav_service_test.dart
  - 批次 5C 播放器全屏行为统一重构（c2c21b5f）：上游将 smallest_player_item_panel（已删除）的职责拆入 player_transport_bar / video_fullscreen_controller / video_page_layout / video_side_panel / video_system_bars；回植 fork 功能——播放页「弹幕」页签（tabController 3 标签）、桌面控制条弹幕时间轴菜单、弹幕设置面板 playerDanmakuController 对接、远程投屏仅在线模式显示、本地媒体/边下边播播放页三模式初始化分支、选集面板本地媒体角标与在线模式专属「缓存剧集」入口
    - 相关文件: lib/pages/player/player_item.dart, lib/pages/player/player_item_panel.dart, lib/pages/player/player_panel_hold.dart, lib/pages/player/player_transport_bar.dart, lib/pages/video/video_page.dart, lib/pages/video/video_controller.dart, lib/pages/video/video_fullscreen_controller.dart, lib/pages/video/video_page_layout.dart, lib/pages/video/video_side_panel.dart, lib/pages/video/video_system_bars.dart, lib/services/platform/display_mode_service.dart, lib/services/platform/platform_environment_service.dart, lib/utils/device.dart, android/app/src/main/kotlin/com/example/kazumi/MainActivity.kt 等
  - 批次 5D 播放器修复与弹幕源选择器重设计：退出时加载指示器不再闪烁（cd9bc04c）、移除重复空状态操作（c94c4830）、弹幕源选择器重设计（a2e5a583，面板菜单入口改用新 showDanmakuSourceSheet，fork 的 showDanmakuSwitchDialog 仍供「弹幕」页签使用；保留 fork 的最后一集播完引导提示）
    - 相关文件: lib/pages/player/player_item.dart, lib/pages/player/danmaku_source_sheet.dart, lib/bean/dialog/dialog.dart, lib/bean/dialog/adaptive_bottom_sheet.dart, lib/bean/dialog/material_bottom_sheet.dart, lib/bean/widget/side_panel_transition.dart, lib/pages/video/video_side_panel.dart, lib/pages/player/episode_comments_view.dart, lib/pages/plugin_editor/plugin_view_page.dart, lib/pages/search/image_search_widgets.dart, lib/pages/video/video_page.dart
  - 批次 5E 收藏库响应式布局与默认视图（80e256ec，含 c24f9a85/4aef9b39/ba21fe35）：卡片/列表双布局 + 可配置默认视图 + hero 圆角与分类过渡修复；回植 fork 的本地可用/缓存角标到新 _CollectPosterCard（封面叠角标）与 _CollectListTile；窗口关闭选项与启动设置交互对齐（c24f9a85）；随上游移除遗留 showAnimeCounter 设置键
    - 相关文件: lib/pages/collect/collect_library_view.dart, lib/pages/collect/collect_library_card.dart, lib/pages/collect/collect_library_controls.dart, lib/modules/collect/collect_layout.dart, lib/pages/collect/collect_page.dart, lib/pages/settings/interface_settings.dart, lib/services/storage/settings_keys.dart, lib/bean/card/network_img_layer.dart
  - 批次 5F 依赖升级：media-kit 全部 git ref 994465d → 6fd002aa（ac494bce/4fed48b5/551b2360/0029b732 终态）；随上游移除 media_kit_libs_* dependency_overrides 与 media_kit_libs_video 直接依赖；Dart SDK 下限提升至 >=3.10.0（Flutter 3.47.2 自带 Dart 3.13.2 满足）；pub get 重生成 pubspec.lock 与 windows/linux/macos 插件注册文件；跳过 Flutter 3.47.3/3.47.5 SDK 升级（保持 3.47.2）、go_router 迁移+回退（净效果为零）、macOS/CI/version 提交
    - 相关文件: pubspec.yaml, pubspec.lock, windows/flutter/generated_plugin_registrant.cc, windows/flutter/generated_plugins.cmake, linux/flutter/generated_plugin_registrant.cc, linux/flutter/generated_plugins.cmake, macos/Flutter/GeneratedPluginRegistrant.swift
  - 回归自查与清理：fork 功能入口逐项核查通过（磁力/本地媒体库/弹幕时间轴偏移/低内存模式/搜索本地匹配横幅/历史缺失本地文件引导/WebDAV 排除本地历史/Bangumi 同步偏好/评论标签编辑/时间线「只看日本动画」筛选）；删除批次 4 起已无引用的孤儿文件 bangumi_history_card.dart；验证 `dart analyze lib test` 0 error / 0 warning（48 条 info 系语言版本升至 Dart 3.10 激活新 lint 规则，属预期基线变化）、`flutter test` 399 项全部通过；Windows 本地构建需在本机终端运行 tools/build_windows_local.ps1 验证
    - 相关文件: lib/bean/card/bangumi_history_card.dart（删除）, UPSTREAM_SYNC.md
