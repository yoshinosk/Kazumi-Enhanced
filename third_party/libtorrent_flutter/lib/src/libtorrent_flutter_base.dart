...
class LibtorrentFlutter {
  ...
  /// Kazumi capability flag: whether the native session was built with
  /// `no_recheck_incomplete_resume = false`, i.e. re-adding a torrent
  /// without resume data triggers a disk file check instead of starting
  /// a full re-download.
  ///
  /// The `prebuilt/` DLLs in this vendored copy are rebuilt from
  /// `src/torrent_bridge.cpp` (Kazumi "重启续做种" plan, verified by
  /// probe E), so this is `true`.
  static const bool resumeAware = true;
